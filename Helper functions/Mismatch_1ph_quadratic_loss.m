function dF = Mismatch_1ph_quadratic_loss(E,S,E_star,S_star,Grid_para,Filter_para,idx)
    ...
    dP_slack_tot = sum((reshape(real(S(idx.slack)),Grid_para.n_ph,length(idx.slack)/Grid_para.n_ph)),2);
    dP_slack_tot = repmat(dP_slack_tot,length(idx.slack)/Grid_para.n_ph,1);
